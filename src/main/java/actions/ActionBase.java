...
//各Actionクラスの親クラス（スーパークラス）。共通処理を行う。
...
public abstract class ActionBase {
    ...
    protected boolean checkToken() throws ServletException, IOException {
        ...
        if (_token == null || !(_token.equals(getTokenId()))) {
            ...
            //tokenが認定されていない、またはセッションIDと一致しない場合はエラー画面を表示
            forward(ForwardConst.FW_ERR_UNKNOWN);
    ...
    //第2引数の型Vはジェネリクス（Generics・総称型）というもの。与えた引数の型がこのVにあたる、という意味。
    //すべての型を引数にとることができる。
    //@param key パラメータ名
    //@param value パラメーの値
    ...
    //セッションにはあらゆる型のオブジェクトを格納できる。そのオブジェクトを取得するため、メソッドの戻り値の型もさまざまになる。
    //戻り値をジェネリクスで記述。
    //@param key パラメータ名
    //@return パラメータの値
