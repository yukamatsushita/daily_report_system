...
public class ReportService extends ServiceBase {
    ...
    //指定した従業員が作成した日報データを、指定されたページ数の一覧画面に表示する分取得しReportViewのリストで返却
    //@param employee 従業員
    //@param page ページ数
    ...
    public long countAllMine(EmployeeView employee) {
        ...
        long count = (long) em.createNamedQuery(JpaConst.Q_REP_COUNT_ALL_MINE, Long.class)
                .setParameter(JpaConst.JPQL_PARM_EMPLOYEE, EmployeeConverter.toModel(employee)).getSingleResult();
    ...
    public List<String> create(ReportView rv) {
        ...
        //バリデーションで発生したエラーのを返却（エラーがなければ0件の空リスト）
        return errors;
    }

    //画面から入力された日報の登録内容を元に、日報テーブルを更新する
    //@param rv 日報の更新内容
    //@return バリデーションで発生したエラーのリスト
