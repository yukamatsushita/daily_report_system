...
@NamedQueries({
        ...
        @NamedQuery(name = JpaConst.Q_EMP_COUNT_RESISTERED_BY_CODE, query = JpaConst.Q_EMP_COUNT_RESISTERED_BY_CODE_DEF),
...
@AllArgsConstructor //全てのクラスフィールドを引数にもつ、引数ありコンストラクタを自動生成する（Lombok）
@Entity
...
public class Employee {
    ...
    //社員番号
    @Column(name = JpaConst.EMP_COL_CODE, nullable = false, unique = true) //一意制約（すでに存在されている社員番号は登録できないことをDBで指定）
    private String code;
    ...
    //パスワード
    @Column(name = JpaConst.EMP_COL_PASS, length = 64, nullable = false) //64文字のハッシュ化された文字列に変換
